...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                flippedVer[i][j] = image[i][cols - 1 - j];
            }
        }
        return flippedVer;
	}
	...
	public static Color[][] grayScaled(Color[][] image) {
		int rows = image.length;
        int cols = image[0].length;
        Color[][] grayS = new Color[rows][cols];
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				int srcRow = i * rows / height;
				int srcCol = j * cols / width;
				arr[i][j] = image[srcRow][srcCol];
			}
		}
		return arr;
	}
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
        ...
        Color[][] scaledT = scaled(target, cols, rows);

        for (int step = 0; step <= n; step++) {
            double alpha = (double) step / n;
            Color[][] blended = blend(source, scaledT, alpha);
            display(blended);
            StdDraw.pause(100); 
        }
	}
